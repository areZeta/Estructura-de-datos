...
pero siempre evitando los ciclos, en esta version implementando el algoritmo de kruskal con el objetivo de iodentificar las ventajas de su desarrollo*/
...
int ingresarGrafo(int c[][30], int , int, lista &grafo, int v){
	int op, costo, menor;
	char x,y;
	struct nodo *aux, *auxPos;
	do{
		cout<<"\n-------------------------------------------------------\n";
		cout<<"Ingresando las aristas (par de elementos relacionados) : \n\n";
            ...
            cin>>y;
            do{
            	cout<<"Ingrese costo del arco entre los vertices anteriores: \n";
                cin>>costo;
                if(costo <=0) cout <<"Error, costo fuera de rango \n";
			}while(costo <= 0);
			menor = costo;
         cout<<"\n-------------------------------------------------------\n";   
		int ban=0;
	    ...
	    if(ban==0) { //indicador de existencia del primer vertice
	     cout<<"El vertice inicial no existe \n"; }else{
	     	struct nodo *conex = grafo; 
	     	for(int i = 0; i < v; i++){
	     		cont2 += 1;
	     		if(y==conex->nombre){   //si el primer vertice existe, se crea el nodo correspondiente al vertice relacionado en el campo ady, que es 
				  break;
			    }
	     		conex = conex->sig;
			 }
			 c[cont][cont2] = costo; // con ayuda de los contadores en la buisqueda en el grafo se obtiernen los indices para agregar el costo a la matriz
			 menor = costo;
		 }
    ...
    //se imprime matriz de costos
    cout<<"\n Matriz de costos entre cada par de vertices conectados \n";
    imprimir(c, 30,30,v);
    aux = grafo;
...
//inicializamos la matriz de costo  v x v dada por el usuario
void initMatrix(int m[][30],int , int, int v){
...
void arbolExpanMin( int expMinima[][30], int, int,int c[][30], int ,int ,lista &grafo, int v, int menor){
	struct nodo *aux = grafo, *aux2 = grafo->ady; //declaracion de la cola y los apuntadores auxiliar
    // recorre la lista de lista para verificar que el grafo sea conexo
    char marca[2][50]; //se declara una matriz de que llevara el control de  los vertices  visitados y los no visitados 
	for(int i = 0; i < v ; i++){ //ciclo que inicializa la matriz de marcas con la letra n señalando que los nodos no han sido visitados
		marca[0][i] = aux->nombre;
	    marca[1][i] = 'n';
	    aux = aux->sig;
    ...
    int cont = 1;
    aux = grafo, marca[1][0] ='c';
    for(int i = 0; i < v; i++){//con el primer ciclo con ayuda de la lista de listas verificamos si la matriz es conexa
    	if(aux->ady != NULL){
    		aux2 = aux->ady; 
    	   while(aux2 !=  NULL){// el segundo ciclo recorre los adyacentes de cada nodo
	    	  for(int j = 0; j < v; j++){
	    	     if((marca[0][j] == aux2->nombre) && (marca[1][j] == 'n')){marca[1][j] ='c'; cont+= 1;} // con un contador verifica que todos los vertices
				 // fueron visitados
			  }
			  aux2 = aux2->sig;
		   }
	...
	bool conexo = false;
	if(cont == v){conexo = true;}
	if(conexo){ // es conexo si  entro a marcar como visitados todos los nodos(esto es que el num de vertices es igual al contador)
		bool end;
		for(int i = 0; i < v ; i++){ //ciclo que inicializa la matriz de marcas con la letra n señalando que los nodos no han sido visitados
		    marca[1][i] = 'n';
        }
        //recorre la matriz para encontrar el arco con el menor costo 
    	int  i, j, auxi, auxj;
        for(i = 0; i < v; i++){//primer ciclo lleva el control de las filas
        	for(j = 0; j < v; j++){//segundo ciclo lleva control de las columnas 
        		if(c[i][j] < menor && c[i][j] != 0 ){
				  menor =  c[i][j];// menor va tomando valores segun encuientr el menor en cada indice
				  auxi =i, auxj =j;
			    }
			}
		}
		//marcar como visitado
		cout<<"\n Arbol de extension minima "; // para el primer arco con costo menor
		marca[1][auxi] = 'v'; // se marca como visitado el nodo en el indice i y  el j se toma como referencia de renglon
		expMinima[auxi][auxj] = 1; //y en la matriz se agrega un uno entre el par de nodos indicando adyacencia
		cout<<"\n "<<marca[0][auxi]; // se imprime el primer nodo(el nodo de donde parte )
		aux = grafo ; end = true; int mayor;
		while(end){ // son el ciclo se busca encontrar un arbol de extension minima
			end = false; 
			//busca costo mayor dentro de un renglon de la matriz a lo largo de sus columnas
			i = auxj; // toma el indice j y se lo asigna a i para que maneje el indice del renglon 
			mayor = 0;
			for(int n = 0; n < v; n++){ 
			...
			for(j = 0; j < v; j++){// una vez encontrado el mayor se le asigna al menor 
				menor = mayor; //la busqueda se hace en la matriz de costos
				if(c[i][j] <= menor && marca[1][j] != 'v' && c[i][j] != 0 ){ // las siguientes restricciones son para
				// verificar que durante el ciclo si se elija al costo menor, pero considerando si ya fue vivitado o no (esto para evitar ciclos) 
				//asi como tambien verificar que el costo en ese arco  no sea 0( el par de nodos no sea adyacente)
					menor = c[i][j];
					 auxj = j;
				    end = true;
				}//else end = false;
			}
			if(end){// en caso de que si se haya encontrado el arco con el costo menor y que haya cumplido las restricciones pertinentes
			  marca[1][i] = 'v'; // el indice i  es el que va a indicar el nodo a marcar como visitado
			  expMinima[i][auxj] = 1;  // se agrega un 1 en la matriz de extension para indicar el nuevo grafo formado
			  cout<<"\n "<<marca[0][i]; // se imprime el nuevo grafo
			}
		}
		cout<<"\n "<<marca[0][auxj]; // el ultimo nodo del arbol minimo
		cout<<" \n( representado con matriz de adyacencia) \n";
		imprimir(expMinima,30,30,v);
	}else{cout<<" \n Grafo no conexo \n";}
}
...
{
	int menor;
	//se define la matriz por default de costos
	 int costos[30][30];
	 int expMinima[30][30];
	lista grafo = NULL;
    ...
    initMatrix(costos,30,30, v);
	initMatrix(expMinima, 30,30,v);
    inicializar(grafo,v);
    ...
    menor = ingresarGrafo(costos,30,30,grafo,v);
    ...
    { 
        ...
        cout<<" 6. ARBOL DE EXPANSION MINIMA                                "<<endl;
        cout<<" 0. SALIR                                                    "<<endl;
        cout<<"\n-------------------------------------------------------\n";
        ...
        switch(op)
        {
            ...
            break;
            case 6:
            	  arbolExpanMin(expMinima, 30, 30,costos, 30, 30, grafo, v,menor);
            break;	  
            case 0:
            break;
            default:
